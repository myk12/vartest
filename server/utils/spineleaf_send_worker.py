...
def socket_send_probes(sender_id, target_ip, namespace, interface, packet_size, count=100, interval=0.1):
    ...
    for seq in range(count):
        # Build probe header
        probe_header = struct.pack(PROBE_HEADER_FORMAT, 0xABCD1234, sender_id, seq, 0)  # hop_count=0 initially
        
        # Build empty hop data for up to 4 hops
        empty_hops = b''.join([struct.pack(HOP_DATA_FORMAT, 0, 0, 0, 0) for _ in range(4)])
        
        # Construct full packet payload
        payload = probe_header + empty_hops + b'7' * (packet_size - len(probe_header) - len(empty_hops))
...
if __name__ == "__main__":
    ...
    if args.use_socket:
        ...
        scapy_send_probes(args.target_mac, args.target_ip, args.sender_id, args.interface, args.count, args.interval)
